...
library SortLinkedList {
    ...
    function improveRanking(List storage _list, IValidator _value) internal {
        //insert new
        if (_list.length == 0) {
            _list.head = _value;
            _list.tail = _value;
        ...
        if (_prev == IValidator(address(0))) {
            ...
            if (_value.pledge_amount() <= _list.tail.pledge_amount()) {
            ...
            if (_value.pledge_amount() <= _prev.pledge_amount()) {
                return;
            }
        ...
        while (_prev != IValidator(address(0)) && _value.pledge_amount() > _prev.pledge_amount()) {
    ...
    function lowerRanking(List storage _list, IValidator _value) internal {
        IValidator _next = _list.next[_value];
        if (_list.tail == _value || _next == IValidator(address(0)) || _next.pledge_amount() <= _value.pledge_amount()) {
            return;
        }
        ...
        while (_next != IValidator(address(0)) && _next.pledge_amount() > _value.pledge_amount()) {
    ...
    function removeRanking(List storage _list, IValidator _value) internal {
