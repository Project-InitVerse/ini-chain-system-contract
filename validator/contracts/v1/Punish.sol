...
contract Punish is Params, IPunish {
    ...
    function punish(address _val) external onlyMiner onlyInitialized onlyNotPunished {
        ...
        if (punishRecords[_val].missedBlocksCounter % removeThreshold == 0) {
            IDposPledge _pool = validatorsContract.dposPledges(_val);
            _pool.punish();
    ...
    function cleanPunishRecord(address _val) external override onlyInitialized {
        require(address(validatorsContract.dposPledges(_val)) == msg.sender, "Validator not registered");
    ...
    //Todo : for test
    /*
    function punisha(IDposPledge cc)external{
        cc.punish();
    }*/
}
